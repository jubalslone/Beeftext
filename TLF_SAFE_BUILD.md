...
Legacy upstream Beeftext `.json` and `.csv` combo exports remain accepted through the same Import command and keep their existing destination-group behavior. The import picker initially shows every supported `.txt`, `.json`, and `.csv` file together, with narrower per-format filters available afterward. Malformed or unsupported files are rejected before the live combo/group data is changed.
